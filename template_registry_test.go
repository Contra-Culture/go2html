		...
		Describe(".Mkdir()", func() {
			Context("when does not exist", func() {
				It("returns dir", func() {
					...
					dir, err := r.Mkdir("1")
					Expect(err).To(BeNil())
					Expect(dir).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
		...
		Describe(".TemplateNode()", func() {
			Context("when template exists", func() {
				It("returns template node", func() {
					...
					Expect(ch1_1.Messages[1]).To(Equal("ok: injection (test.test-class)"))
					Expect(ch1_1.Messages[2]).To(Equal("ok"))
					Expect(ch1_1.Messages[3]).To(Equal("ok: injection (test.nested-template-text)"))
					Expect(ch1_1.Messages[4]).To(Equal("ok"))
					Expect(ch1_1.Messages[5]).To(Equal("ok: injection (test.text)"))
			...
			Describe(".Mkdir()", func() {
				...
				Context("when exists", func() {
					It("fails and returns error", func() {
						...
						dir1, err := r.Mkdir("1")
						Expect(err).To(BeNil())
						Expect(dir1).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
						dir1_1, err := dir1.Mkdir("1-1")
						Expect(err).NotTo(HaveOccurred())
						Expect(dir1_1).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
						dir1_1, err = dir1.Mkdir("1-1")
						Expect(err).To(HaveOccurred())
						Expect(err.Error()).To(Equal("directory \"1-1\" already exists"))
						...
						Expect(err).To(BeNil())
						Expect(dir1_2).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
						dir1_2, err = dir1.Mkdir("1-2")
						Expect(err).To(HaveOccurred())
			...
			Describe(".Dir()", func() {
				Context("when exists", func() {
					It("returns dir", func() {
						...
						Expect(err).NotTo(HaveOccurred())
						Expect(dir).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
						dir1_1, err := dir.Dir("1-1")
						Expect(err).NotTo(HaveOccurred())
						Expect(dir1_1).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
						dir1_2, err := dir.Dir("1-2")
						Expect(err).NotTo(HaveOccurred())
						Expect(dir1_2).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
						dir1_1_1, err := dir.Dir("1-1", "1-1-1")
						Expect(err).NotTo(HaveOccurred())
						Expect(dir1_1_1).To(BeAssignableToTypeOf(&TemplateRegistryDir{}))
					})
				})
				Context("when does not exist", func() {
					It("fails and returns error", func() {
